...
def _synthetic_candidates(seed=2025, n_users=1000):
    features=build_feature_frame(generate_marketplace(seed, n_users))
    eligible=features[features.is_eligible].copy()
    rng=np.random.default_rng(seed)
    rows=[]
    for row in eligible.itertuples():
        base=max(0.001, 0.01 + 0.015*float(row.engagement_score))
        lift=max(-0.004, 0.006*float(row.engagement_score) - 0.002*float(row.days_since_last_completed_order)/30)
        for action, multiplier, face, redemption in [("no_offer",0,0,0),("small_offer",1,50,.35),("large_offer",1.35,100,.55)]:
            inc=lift*multiplier
            cost=face*redemption
            segment="active" if row.engagement_score >= .35 else "lapsing" if row.engagement_score >= .15 else "dormant"
            rows.append({"user_id":row.user_id,"action":action,"incremental_conversion":inc,"expected_margin":176.0,"expected_cost":cost,"expected_value":inc*176-cost,"propensity_score":base,"city_hour":f"{row.city_id}_{int(row.preferred_hour)}","expected_incremental_orders":max(0,inc),"segment":segment})
...
def run(mode: str = "smoke", output_dir: str | Path = "reports", criteo_path: str | Path | None = None, seed: int = 2025) -> dict[str, object]:
    ...
    budgets=[]; candidates=_synthetic_candidates(seed,1000)
    baseline_cost=float(candidates[candidates.action=="small_offer"].expected_cost.sum()); grid=[.1,.25,.5,.75,1.0]
    for fraction in grid:
        result=optimize_allocation(candidates,baseline_cost*fraction,maximum_contact_volume=max(1,int(candidates.user_id.nunique()*.5)),return_shadow=True)
        budgets.append({"budget_fraction":fraction,"budget_inr":baseline_cost*fraction,"expected_value":result.expected_value,"expected_cost":result.expected_cost,"utilization":result.utilization,"shadow_price_proxy":result.marginal_budget_value})
    budgets_frame=pd.DataFrame(budgets); budgets_frame.to_csv(output/"policy_comparison.csv",index=False)
    policy_rows=[]
    for fraction in grid:
        ...
        optimized=budgets_frame.iloc[len(policy_rows)//6-1]
        policy_rows.append({"policy":"optimized","budget_fraction":fraction,"budget_inr":budget,"expected_value":float(optimized.expected_value),"expected_cost":float(optimized.expected_cost),"treatment_rate":float(optimized.expected_cost/baseline_cost)})
    ...
    fig,ax=plt.subplots(); ax.plot(budgets_frame.budget_inr,budgets_frame.expected_value,"o-"); ax.set(xlabel="Budget (INR)",ylabel="Expected net value (INR)",title="Simulated policy value vs budget"); fig.tight_layout(); profit_path=figures/"profit_vs_budget.png"; fig.savefig(profit_path,dpi=140); plt.close(fig)
    ...
    paragraphs=[f"Label: {'measured' if mode=='full' else 'simulated smoke'} Criteo-shaped randomized advertising data.",f"Conversion ITT estimate: {estimate.point:.6f} [{estimate.ci_low:.6f}, {estimate.ci_high:.6f}], n={estimate.treated_n+estimate.control_n}.",f"Held-out policy value is estimated by inverse-probability weighting. Synthetic multi-action allocation is simulated and not a realized business result."]
    _write_pdf(output/"experiment_readout.pdf","Experiment readout",paragraphs,[("Estimated Qini",plt.imread(qini_path))])
    _write_pdf(Path("docs")/"executive_case_study.pdf","Increment executive case study",["Decision: allocate a fixed promotion budget to maximize incremental contribution margin per eligible customer.",f"The smoke run estimates a conversion ITT of {estimate.point:.4f}; uncertainty is [{estimate.ci_low:.4f}, {estimate.ci_high:.4f}].",f"The canonical 50% synthetic budget produces expected net value of {budgets_frame.iloc[2].expected_value:.0f} INR. This is simulated, not realized impact.","Recommendation: use the constrained policy only behind the persistent holdout and guardrail gates described in docs/rollout_plan.md."],[("Estimated Qini",plt.imread(qini_path)),("Simulated profit curve",plt.imread(profit_path))])
    deck_path=output/"interview_deck.pptx"; _write_deck(deck_path,f"Canonical synthetic budget result: {budgets_frame.iloc[2].expected_value:.0f} INR expected value at 50% budget",[("Qini",qini_path),("Uplift deciles",decile_path),("Profit vs budget",profit_path)])
    manifest={"mode":mode,"seed":seed,"rows":len(frame),"estimate":estimate.__dict__,"policy_value_estimate":policy_point,"platform":platform.python_version(),"inputs":{}}
