...
from src.reporting.claims import write_claim_registry
...
def run(mode: str = "smoke", output_dir: str | Path = "reports", criteo_path: str | Path | None = None, seed: int = 2025) -> dict[str, object]:
    ...
    # Rewrite the case study with the same run-mode label used by the claim
    # registry; this prevents a full-data artifact from inheriting smoke text.
    ...
    manifest["claim_registry_path"] = str(registry_path)
